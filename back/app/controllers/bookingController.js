...
const bookingController = {
  createNewBooking: async (req, res) => {
    ...
    const disponibilityExist = await bookingDatamapper.getOneUserDisponibility(
      petSitterId,
      newBooking.disponibility_id
    );
    ...
    if (!disponibilityExist) {
      res
        .status(401)
        .json({ message: "Aucune disponibilité n'a été enregistré" });
      return;
    }
...
<<<<<<< HEAD
    ...
    if (
      disponibilityExist.start_date.toLocaleDateString('fr-FR') ==
        startDateString &&
      disponibilityExist.end_date.toLocaleDateString('fr-FR') == endDateString
    ) {
      disponibilityExist.start_date =
        disponibilityExist.start_date.toLocaleDateString('fr-FR', {
          day: 'numeric',
          month: 'numeric',
          year: 'numeric',
        });
      disponibilityExist.end_date =
        disponibilityExist.end_date.toLocaleDateString('fr-FR', {
          day: 'numeric',
          month: 'numeric',
          year: 'numeric',
        });

      const booking = await bookingDatamapper.addBooking(
        petSitterId,
        userId,
        newBooking
      );
    ...
    if (
      ...
      let newDisponibilityStartDate = new Date(
        endDate.setDate(endDate.getDate() + 1)
      );
      newDisponibilityStartDate = newDisponibilityStartDate
        .toLocaleDateString('fr-FR')
        .split('/')
        .reverse()
        .join('-');
      // Formatage de la date de fin de dispo inchangé
      const disponibilityEndDate = disponibilityExist.end_date
        .toLocaleDateString('fr-FR')
        .split('/')
        .reverse()
        .join('-');
      ...
      const newPetsitterDisponibility =
        await userDatamapper.addNewUserDisponibilitites(
          newDisponibilityStartDate,
    ...
    if (
      ...
      const disponibilityStartDate = disponibilityExist.start_date
        .toLocaleDateString('fr-FR')
        .split('/')
        .reverse()
        .join('-');
      // Nouvelle date de fin de dispo
      let newDisponibilityEndDate = new Date(
        startDate.setDate(startDate.getDate() - 1)
      );
      newDisponibilityEndDate = newDisponibilityEndDate
        .toLocaleDateString('fr-FR')
        .split('/')
        .reverse()
        .join('-');
    ...
    if (
      ...
      newDisponibilityEndDate = newDisponibilityEndDate
        ...
        .join('-');
      // Création de la dispo en amont de la résa
      const newPetsitterDisponibilityUpstream =
        await userDatamapper.addNewUserDisponibilitites(
          disponibilityStartDate,
          newDisponibilityEndDate,
          petSitterId
      ...
      const newPetsitterDisponibilityDownstream =
        await userDatamapper.addNewUserDisponibilitites(
          newDisponibilityStartDate,
          disponibilityEndDate,
          petSitterId
