...
const userController = {
  ...
  createUser: async (req, res) => {
    ...
    try {
      ...
      if (userExist) {
        res
          .status(401)
          .json('Cet email est déjà utilisé ! Veuillez vous logger');
        return;
      }
  ...
  findUserByDistance: async (req, res) => {
    ...
    try {
      const users = await userDatamapper.getUsersByDistance(
        searchParameters,
        userId
      );
      res.json(users);
  ...
  createUserNewDisponibility: async (req, res) => {
    try {
      ...
      res.json({
        start_date: start_date_front,
        end_date: end_date_front,
        message: 'Vous venez de créer une disponibilité',
      });
    } catch (error) {
      res.status(404).json('Erreur de connexion server');
    }
  ...
  updateDisponibility: async (req, res) => {
    ...
    const findDisponibilityByUserId = await userDatamapper.getUserDisponibility(
      userId
    );

    // Si elle existe, on modifie les dates de disponibilités
    if (findDisponibilityByUserId) {
      const updateDisponibility = await userDatamapper.modifyDisponibility(
        findDisponibilityByUserId[0].id,
        startDateFormat,
        endDateFormat,
        userId
      );
      if (updateDisponibility) {
        return res.json({
          ...
          message: 'Vos disponibilités ont bien été modifié',
        });
      }
    }
  },
  ...
  updateOurAnimal: async (req, res) => {
    ...
    try {
      const ourNewAnimal = await userDatamapper.modifyOurAnimal(
        newAnimal,
        userId
      );
      ...
      if (ourNewAnimal) {
        return res.json({
          message: 'Votre animal a bien été modifié',
          animal: ourNewAnimal,
        });
      }
    } catch (error) {
