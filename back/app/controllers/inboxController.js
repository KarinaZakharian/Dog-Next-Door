...
const inboxController = {
    ...
    try {
      ...
      const userMessageList = userMessage.map((message) => {
        message.booking.start_date = message.booking.start_date
          ...
          .reverse()
          .join('/');
        message.booking.end_date = message.booking.end_date
          .split('-')
          .reverse()
          .join('/');
        return message;
  ...
  findBookingRequest: async (req, res) => {
    ...
    try {
      ...
      const userMessage = await inboxDatamapper.getBookingRequest(
        userId,
        searchPetSitter.user_id
      );
  ...
  acceptBooking: async (req, res) => {
    ...
    try {
      if (information.answer == 'true') {
        const bookingAccept = await inboxDatamapper.bookingAccepted(
        ...
        if (bookingAccept) {
          return res
            .status(200)
            .json({ message: 'Vous venez de valider votre réservation' });
        }
      } else {
  ...
  findUpcomingBooking: async (req, res) => {
    ...
    try {
      ...
      const userMessage = await inboxDatamapper.getUpcomingBooking(
        id,
        userRequest
      );
  ...
  findPastBooking: async (req, res) => {
    ...
    try {
      ...
      const userMessage = await inboxDatamapper.getPastBooking(
        id,
